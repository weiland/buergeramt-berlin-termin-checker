...
// TODO(pascal): Need to make sure that the url will always work
...
function notify(ms) {
  ...
  var msg = {
    message: 'just found an appointment for flat registration: ' + ms,   // required
...
function getMonth(td) {
  var ret;
  try {
    ret = td.parent.parent.prev.prev.children[1].children[2].next.children[0].data.trim().replace(YEAR_REGEX, '');
  } catch(e) {
    console.log(td);
  }
  return ret;
